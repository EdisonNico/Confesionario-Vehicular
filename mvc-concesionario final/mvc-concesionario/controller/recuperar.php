...
if ($correo && $nueva_contrasenia) {
    try {
        ...
        if ($usuario) {
            ...
            $update = $conexion->prepare("UPDATE usuarios SET contrasenia = ? WHERE correo = ?");
...
<head>
    ...
    <style>
        * {
        ...
        .modal {
            background: rgb(22, 8, 59); /* Cuadro blanco semitransparente */
            padding: 30px 40px;
            border-radius: 15px;
            text-align: center;
            ...
            backdrop-filter: blur(8px); /* Efecto elegante */
        }
